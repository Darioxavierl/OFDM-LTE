...
class MetricsPanel(QWidget):
    ...
    def init_ui(self):
        ...
        # Labels para métricas (BER, PAPR, Rank, Tiempo)
        self.ber_label = QLabel("BER: --")
        self.papr_label = QLabel("PAPR: -- dB")
        self.gain_label = QLabel("Rank: --")
    ...
    def update_metrics(self, metrics):
        """Actualiza las métricas mostradas (BER, PAPR, Rank, Tiempo)"""
        if 'ber' in metrics:
            self.ber_label.setText(f"BER: {metrics['ber']:.2e}")
        if 'papr_db' in metrics:
            self.papr_label.setText(f"PAPR: {metrics['papr_db']:.2f} dB")
        if 'rank_used' in metrics:
            self.gain_label.setText(f"Rank: {metrics['rank_used']}")
